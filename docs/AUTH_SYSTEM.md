...
> **Dev mode note:** when `VITE_NODE_ENV !== "production"`, cross-subdomain redirects (both cookie-based and user-based) are skipped — the SPA stays on the dev origin and sends mismatched visitors to the local `/login` route instead of another subdomain.
