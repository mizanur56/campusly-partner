...
export function transformFiltersToApi(
  ...
  if (filterData.destination?.length > 0 && countriesResponse?.data) {
    const countryMap = new Map(
      countriesResponse.data.filter((c) => c.isActive).map((c) => [c.name, c.id])
    );
    ...
    if (filteredLevels.length > 0) {
      const studyLevelMap = new Map(
        studyLevelsResponse.data
          ...
          .map((sl) => [sl.name, sl.id])
      );
      const studyLevelIds = filteredLevels
        .map((name) => studyLevelMap.get(name))
        .filter((id): id is string => !!id);
  ...
  if (filterData.duration?.length > 0) {
    ...
    const allMins = durations.map((d) => d.min).filter((n): n is number => n !== undefined);
    const allMaxs = durations.map((d) => d.max).filter((n): n is number => n !== undefined);
  ...
  if (filterData.startMonth?.length) {
    const monthNum = getMonthNumber(filterData.startMonth[0]);
...
export function buildSearchQueryString(params: ApiSearchParams): string {
  ...
  if (params.countryIds?.length) queryParams.append("countryIds", params.countryIds.join(","));
  if (params.cityIds?.length) queryParams.append("cityIds", params.cityIds.join(","));
  if (params.universityIds?.length) queryParams.append("universityIds", params.universityIds.join(","));
  if (params.courseIds?.length) queryParams.append("courseIds", params.courseIds.join(","));
  if (params.studyLevelIds?.length) queryParams.append("studyLevelIds", params.studyLevelIds.join(","));
  if (params.durationMin !== undefined) queryParams.append("durationMin", params.durationMin.toString());
  if (params.durationMax !== undefined) queryParams.append("durationMax", params.durationMax.toString());
  if (params.feeMin !== undefined) queryParams.append("feeMin", params.feeMin.toString());
  if (params.feeMax !== undefined) queryParams.append("feeMax", params.feeMax.toString());
  if (params.startYear !== undefined) queryParams.append("startYear", params.startYear.toString());
  if (params.startMonth !== undefined) queryParams.append("startMonth", params.startMonth.toString());
