        ...
        {isLoading && universities.length === 0
          ? Array.from({ length: 5 }).map((_, idx) => (
              <div key={idx} className="flex items-center gap-2 animate-pulse">
                <div className="h-8 w-8 shrink-0 rounded-2xl bg-gray-100 dark:bg-gray-800" />
          ...
          : universities.map((u) => (
              <div key={u.id} className="flex items-center gap-2">
                <div className="flex h-8 w-8 shrink-0 items-center justify-center overflow-hidden rounded-2xl bg-gray-100 text-xs text-gray-500 dark:bg-gray-800 dark:text-gray-400">
                  {u.logoUrl ? (
                    <img
