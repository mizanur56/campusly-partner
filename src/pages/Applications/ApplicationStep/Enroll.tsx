...
const Enroll: React.FC = () => {
  ...
  const tuitionFeeInvoice = applicationApiData?.invoices?.find(
    (invoice: any) => invoice.type === "TUITION_FEE_FULL_BEFORE" || invoice.type === "TUITION_FEE_HALF_BEFORE"
  );
  ...
  React.useEffect(() => {
    const fetchSizes = async () => {
      ...
      if (applicationApiData?.tuitionDepositReceipt) sizes.tuitionDepositReceipt = await getFileSize(applicationApiData.tuitionDepositReceipt);
      if (applicationApiData?.airticket) sizes.airticket = await getFileSize(applicationApiData.airticket);
      if (applicationApiData?.travelLetter) sizes.travelLetter = await getFileSize(applicationApiData.travelLetter);
      setFileSizes(sizes);
    };
  ...
  const sections = React.useMemo(
    () => [
      ...
      },
      {
        ...
        category: "airticket",
        hasUploaded: true,
        description: "Sponsor's bank statement for the last 3 months, along with a recent bank certificate, is attached. The ending balance should be converted into Euros.",
        url: applicationApiData?.airticket,
  ...
  const isAllRequiredCompleted = sections.every((section) => !!section.isCompleted);
  ...
  return (
    <>
      <div className="border border-[#C7CACF] rounded-lg overflow-hidden">
        <div className="bg-[#E9F2EB] p-6 flex items-center justify-between">
          <div>
            ...
            <p className="text-[14px] text-[#4B5563]">Your documents has been submitted. Your documents are under review, we will send it to the college.</p>
          </div>
        ...
        {isExpanded && (
          <div className="space-y-4 p-4">
            {sections.map((section) => {
              ...
              return (
                <div key={section.id} className="bg-white border border-[#C7CACF] rounded-xl p-6">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    {section.hasUploaded &&
                      (section.id === "tuition_deposit_receipt" ? (
                        <button
                          onClick={() => {
                            if (tuitionFeeInvoice) setIsPaymentModalOpen(true);
                            else toast.error("Tuition fee invoice not found");
                          }}
                  ...
                  <p className="text-[14px] text-[#4B5563] mb-6 leading-relaxed">{section.description}</p>
                    ...
                    <div>
                      ...
                      <div className="flex items-center justify-between border border-[#D1D5DB] rounded-lg p-4 w-fit min-w-70">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-[14px] font-medium text-[#20242A]">{section.title}.pdf</p>
      ...
      <div className="flex justify-end gap-3 pt-4">
        <button onClick={() => navigate(`/applications/${id}/visa`)} className="px-6 py-2 border border-[#D1D5DB] rounded-lg text-[#237D3B] font-semibold hover:bg-gray-50">
          Previous
        </button>
      ...
      {tuitionFeeInvoice && (
        <PaymentReceiptModal open={isPaymentModalOpen} onClose={() => setIsPaymentModalOpen(false)} invoice={tuitionFeeInvoice} />
      )}
    </>
  );
};
