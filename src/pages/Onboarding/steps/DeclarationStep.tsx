...
export default function DeclarationStep({ apiStep, onPrev, onSubmit }: Props) {
  ...
  const hasPersistedData = useMemo(() => step5HasPersistedData(payload), [payload]);
  const {
    formDisabled,
    showEditControl,
    showSaveButton,
    ...
    <>
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0 flex-1 space-y-3">
          <p className="text-[16px] font-medium text-[#20242A] dark:text-white">
            Minor mistakes can cause a major delay in our partnership. Please take a moment to verify the information in each section before submitting.
          </p>
      ...
      >
        <Form.Item name="verifyInfo" valuePropName="checked" rules={[{ required: true, message: "Required" }]}>
          <Checkbox disabled={formDisabled} className="[&_.ant-checkbox-inner]:rounded [&_.ant-checkbox-checked_.ant-checkbox-inner]:bg-primary-500 [&_.ant-checkbox-checked_.ant-checkbox-inner]:border-primary-500">
